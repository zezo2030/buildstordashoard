...
export const appSettingMeta: Record<string, { label: string; hint: string; kind: 'number' | 'text' | 'phone' | 'boolean' }> = {
  ...
  return_refund_grace_hours: {
    label: 'مهلة رد قيمة المرتجع (ساعات)',
    hint: 'بعد ما البائع يستلم البضاعة المرتجعة بكام ساعة لازم يكون رجّع قيمتها للمشتري. عدّت المهلة ⇒ عرض منتجاته بيقف لحد ما يسدّد، وبيرجع تلقائي بالسداد',
    kind: 'number',
  },
  // الإيقاف التلقائي للبائع على مستحقات المنصة — أيهما يحصل الأول.
  seller_suspend_after_days: {
    label: 'إيقاف البائع بعد (أيام)',
    hint: 'لو أقدم مستحق على البائع للمنصة (عمولة كاش/آجل أو اشتراك) عدّى عليه الأيام دي من غير سداد ⇒ عرض منتجاته بيقف تلقائي، وبيرجع لوحده أول ما يسدّد. صفر = الشرط ده مقفول',
    kind: 'number',
  },
  seller_suspend_max_debt: {
    label: 'إيقاف البائع عند مستحق (د.ك)',
    hint: 'أول ما المستحق على البائع للمنصة يوصل المبلغ ده ⇒ عرض منتجاته بيقف فورًا، وبيرجع لوحده أول ما ينزل عنه. صفر = الشرط ده مقفول',
    kind: 'number',
  },
  seller_suspend_min_debt: {
    label: 'أقل مستحق يتوقف عليه البائع (د.ك)',
    hint: 'مستحق أقل من كده ما بيوقفش البائع مهما عدّى عليه وقت — عشان الكسور الصغيرة',
    kind: 'number',
  },
  seller_payment_grace_days: {
    label: 'مهلة سداد كشف الحساب الشهري (أيام)',
    hint: 'تاريخ الاستحقاق المكتوب على الكشف الشهري اللي بيتبعت للبائع. الإيقاف نفسه بالإعدادين اللي فوق',
    kind: 'number',
  },
  seller_statement_day: {
    label: 'يوم إصدار كشف البائع الشهري',
    hint: 'يوم في الشهر بيتصدر فيه كشف الحساب ويتبعت للبائع',
    kind: 'number',
  },
...
/**
 * «عند كام بائع» — عدد عروض البائعين على المادة بصيغة عربية سليمة.
 *
 * العدد هو نفسه اللي في عمود «عروض البائعين» في شاشة المنتجات: كل عروض
 * البائعين على المادة، المفعّل والموقوف — عشان الرقمين ما يختلفوش.
 */
export function sellersOffersLabel(n: number): string {
  if (n <= 0) return 'مفيش بائعين';
  if (n === 1) return 'عند بائع واحد';
  if (n === 2) return 'عند بائعين';
  // العربي بيرجع للمفرد من 11 وطالع: «٣ بائعين» بس «11 بائع».
  return n <= 10 ? `عند ${n} بائعين` : `عند ${n} بائع`;
}
